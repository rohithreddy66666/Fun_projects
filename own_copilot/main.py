...
FUNCTIONS = [
    {
        ...
        "description": ("Semantically search the codebase for relevant methods and "
                        "get the 3 most relevant ones back. This is useful for when "
...
def handle_function_call(resp_function_call, collection, messages):
    ...
    try:
        ...
        messages.append({
            "role": "Rohiths ai assistant",
            "content": f"Here are 3 relevant code pieces from the codebase, formatted by file_name + class_name + method_name, when class name is available. Use these to construct your response to me, and make sure to list relevant code. \n {string_results}"
        })
        ...
        print(colored(f"Error occurred while searching codebase: {e}", "red"))
        return "An error occurred while searching the codebase. Please try again later.", messages

def chat_interaction(collection, readme_txt):
    messages = [
        {
            "role":"system",
            "content": f"You are a helpful code assistant that is developed by rohith that will be answering questions about a very important large codebase. \
